...
class Evaluator:
    ...
    def get_precision(self):
        pr = self.get_precision_per_class()
    ...
    def get_recall(self):
        rc = self.get_recall_per_class()
        return sum(rc.values()) / (len(rc))

    def get_f_score_per_class(self):
        fpc = {}

        rpc = self.get_recall_per_class()
        ppc = self.get_precision_per_class()

        for key in rpc:
            if ppc[key] + rpc[key] > 0:
                fpc[key] = (2.0 * ppc[key] * rpc[key]) / (ppc[key] + rpc[key])
            else:
                fpc[key] = 0
